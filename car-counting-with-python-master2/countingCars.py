...
planWorkBook = xlsxwriter.Workbook('aracSayma.xlsx')  # name of the file to be saved
...
class Sensor:
    def __init__(self,Koordinat1,Koordinat2,Kare_Genislik,Kare_Yükseklik):
       ...
       self.Algılanan_Uzun_Arac_Sayısı = 0
...
Sensor1 = Sensor(
    Koordinat(800, Kesilmis_Kare.shape[1] - 830),
    Koordinat(1200, Kesilmis_Kare.shape[1] - 820),
    Kesilmis_Kare.shape[0],
    Kesilmis_Kare.shape[1])

Sensor2 = Sensor(
    Koordinat(0, Kesilmis_Kare.shape[1] - 840),
    Koordinat(275, Kesilmis_Kare.shape[1] - 830),
    Kesilmis_Kare.shape[0],
    Kesilmis_Kare.shape[1])
...
while (1):
    ...
    mask1=np.zeros((Doldurulmus_Resim.shape[0],Doldurulmus_Resim.shape[1],1),np.uint8)
   ...
   #print(Sensor2_Oran)
    ...
    if (Sensor1_Oran>=0.5 and Sensor1.Durum==False):
        ...
        Sensor1.Durum = False
        Sensor1.Algılanan_Uzun_Arac_Sayısı += 1    
    else :
        cv2.rectangle(Sonuc, (Sensor1.Koordinat1.x, Sensor1.Koordinat1.y), (Sensor1.Koordinat2.x, Sensor1.Koordinat2.y),
                      (0, 0, 255), thickness=cv2.FILLED)
    ...
    if (Sensor2_Oran>=0.5 and   Sensor2.Durum==False):
        ...
        cv2.rectangle(Sonuc, (Sensor2.Koordinat1.x, Sensor2.Koordinat1.y), (Sensor2.Koordinat2.x, Sensor2.Koordinat2.y),
                      (0, 255,255), thickness=cv2.FILLED)
        Sensor2.Durum = False
        Sensor2.Algılanan_Uzun_Arac_Sayısı += 1
    else :
        cv2.rectangle(Sonuc, (Sensor2.Koordinat1.x, Sensor2.Koordinat1.y), (Sensor2.Koordinat2.x, Sensor2.Koordinat2.y),
                      (0, 0, 255), thickness=cv2.FILLED)
    ...
    cv2.putText(Sonuc,str(Sensor1.Algılanan_Uzun_Arac_Sayısı),(Sensor1.Koordinat1.x,250),font,2,(255,255,255))
    cv2.putText(Sonuc,str(Sensor2.Algılanan_Arac_Sayisi),(Sensor2.Koordinat1.x,150),font,2,(255,255,255))
    cv2.putText(Sonuc,str(Sensor2.Algılanan_Uzun_Arac_Sayısı),(Sensor2.Koordinat1.x,250),font,2,(255,255,255))
    ...
    if now < video_bölme_bitis_zamanı :   # We've added our interval to video_bölme_bitis_zamanı and we're constantly increasing it as the time gets closer to it.
      ...
      gelen_uzun_arac_sayisi.append(Sensor2.Algılanan_Uzun_Arac_Sayısı)
      giden_uzun_arac_sayisi.append(Sensor1.Algılanan_Uzun_Arac_Sayısı)
    ...
    if k == 27 :
        ...
        print(gelen_uzun_arac_sayisi)
        print(giden_uzun_arac_sayisi)
